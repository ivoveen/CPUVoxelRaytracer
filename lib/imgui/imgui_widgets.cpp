...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    const float mRadius = (square_sz - 1.0f) * 0.5f;
    ...
    RenderNavHighlight(total_bb, id);
    window->DrawList->AddCircleFilled(center, mRadius, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), 16);
    if (active)
    {
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        window->DrawList->AddCircleFilled(center, mRadius - pad, GetColorU32(ImGuiCol_CheckMark), 16);
    }

    if (style.FrameBorderSize > 0.0f)
    {
        window->DrawList->AddCircle(center + ImVec2(1, 1), mRadius, GetColorU32(ImGuiCol_BorderShadow), 16, style.FrameBorderSize);
        window->DrawList->AddCircle(center, mRadius, GetColorU32(ImGuiCol_Border), 16, style.FrameBorderSize);
    }
...
static ImVec2 InputTextCalcTextSizeW(ImGuiContext* ctx, const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining, ImVec2* out_offset, bool stop_on_new_line)
{
    ...
    const float mScale = line_height / font->FontSize;
    ...
    while (s < text_end)
    {
        ...
        const float char_width = font->GetCharAdvance((ImWchar)c) * mScale;
