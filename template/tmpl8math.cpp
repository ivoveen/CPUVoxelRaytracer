...
thread_local static uint seed = 0x12345678;
...
{
	float frequency = 1; // (float)(2 << numOctaves);
	float amplitude =1;
	int i = 0;
	...
	float noise = (noiseXY + noiseXZ + noiseYZ + noiseYX + noiseZX + noiseZY) * amplitude;

	// Normalize the noise value to the range [0, 1]
	noise = (noise + 1.0f) * 0.5f; // Shift and scale to [0, 1]
	if (noise > 1) {
		noise = 1;
	}
	if (noise < 0) {
